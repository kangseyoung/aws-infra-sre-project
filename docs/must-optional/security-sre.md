...
* EC2 앱 포트는 ALB Security Group에서 오는 요청만 허용
...
* Nginx access log 확인
* Nginx error log 확인
* Application log 위치 확인
* EC2 내부에서 앱 또는 `/health` 응답 확인

결과물:

* Docker / Nginx / App 로그 확인 방법 정리
...
8. Health Check Path 확인
9. Docker / Nginx / MiniPEP 앱 실행 상태 확인
10. EC2 내부 `curl` 테스트
11. Docker / Nginx / App 로그 확인

최종 성공 기준:

* ALB DNS를 통해 MiniPEP 앱이 정상 응답
* 또는 `/health` 요청에 `200 OK` 응답
...
또한 서비스 배포 후 문제가 발생했을 때 Docker, Nginx, App 상태와 로그를 확인하고 정해진 순서대로 대응할 수 있어야 합니다.
...
* Observability / Platform: CloudWatch Metric, Target Health, Docker / Nginx / App 로그 확인
...
로그는 Must에서 EC2 내부의 `docker logs`, Nginx access/error log, Application log를 확인할 수 있는 수준으로 정했습니다. CloudWatch Agent를 설치하고 로그를 CloudWatch Logs로 보내는 것은 Optional입니다.

장애 대응은 ALB, Target Group, EC2, Security Group, 앱 포트, Health Check, Docker, Nginx, MiniPEP 앱, 로그 순서로 확인하는 공통 Runbook 1개를 작성합니다.
