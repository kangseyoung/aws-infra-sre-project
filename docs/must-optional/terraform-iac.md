...
Observability / Platform 파트에서 사용하는 AWS 리소스를 Terraform 코드로 재현하거나 필요 시 terraform import를 고려하고, 필요한 정보를 Output으로 제공한다.
...
- VPC, Public Subnet, Route Table, Internet Gateway, Security Group, EC2, ALB, Target Group 등을 관리한다.
- 리소스 이름, CIDR, Security Group 규칙 등을 협의하고 Variable을 활용하여 변경 사항을 쉽고 동일하게 반영할 수 있도록 구성한다.

### Observability / Platform

- CloudWatch 등 모니터링 대상이 되는 AWS 리소스를 Terraform 코드로 재현하거나 필요 시 terraform import를 고려한다.
- EC2 Instance ID, ALB DNS, Target Group ARN 등 운영에 필요한 정보를 Output으로 제공한다.
- 시간이 남으면 CloudWatch Dashboard, Alarm 등의 리소스도 Terraform으로 코드화한다.
