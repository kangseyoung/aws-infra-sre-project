...
이번 프로젝트에서는 Compute + Network, Observability / Platform, Security + SRE 파트에서 정의한 AWS 리소스와 설정을 Terraform으로 코드화하여 동일한 환경을 반복해서 생성하고 관리할 수 있도록 하는 것을 목표로 한다.
...
Compute + Network 파트에서 정의한 핵심 AWS 리소스를 Terraform으로 관리한다.
...
Observability / Platform 파트에서 사용하는 AWS 리소스를 Terraform으로 관리하고, 필요한 정보를 Output으로 제공한다.

예시

- CloudWatch 관련 리소스(가능한 범위)
- EC2 Instance ID
- ALB DNS
- Target Group ARN
- Output 구성
...
Security + SRE 파트에서 사용하는 보안 관련 리소스를 Terraform으로 관리한다.
...
이번 프로젝트에서는 Compute + Network, Observability / Platform, Security + SRE 파트에서 정의한 핵심 AWS 리소스와 설정을 Terraform으로 코드화하여 프로젝트 전체가 일관된 환경에서 동작할 수 있도록 하는 것이 중요하다고 판단하였다.
...
Module, Remote Backend, Auto Scaling, CloudWatch Alarm, GitHub Actions 자동화 등의 기능은 실제 운영 환경에서는 중요하지만, 2주 프로젝트 안에 모두 적용하기에는 범위가 넓다.
...
- VPC, Subnet, Route Table, Internet Gateway, Security Group, EC2, ALB, Target Group 등을 관리한다.
- 리소스 이름, CIDR, Security Group 규칙 등을 협의하고 Variable을 활용하여 변경 사항을 쉽고 동일하게 반영할 수 있도록 구성한다.

### Observability / Platform

- CloudWatch 등 모니터링 대상이 되는 AWS 리소스를 Terraform으로 관리하거나 필요한 Output을 제공한다.
- EC2 Instance ID, ALB DNS, Target Group ARN 등 운영에 필요한 정보를 Output으로 제공한다.
- 시간이 남으면 CloudWatch Dashboard, Alarm 등의 리소스도 Terraform으로 관리한다.

### Security + SRE

- Security Group, IAM 등 보안 관련 리소스를 Terraform으로 관리한다.
...
이번 프로젝트에서는 Compute + Network, Observability / Platform, Security + SRE 파트에서 정의한 핵심 AWS 리소스와 설정을 Terraform으로 코드화하는 것을 Must 범위로 잡았습니다.

또한 Variable과 Output을 정리하여 팀원들이 동일한 인프라 구성을 함께 관리할 수 있도록 하고, 다른 파트에서 필요한 AWS 리소스 정보를 쉽게 활용할 수 있도록 연결하는 것도 목표입니다.

Terraform은 특정 파트만 담당하는 것이 아니라, 각 파트에서 정의한 Must 범위를 취합하여 일관된 IaC 코드로 관리하는 역할이라고 생각합니다. 따라서 리소스 이름, 네트워크 구성, 보안 정책, 모니터링 대상 등이 변경되면 이를 Terraform 코드에도 함께 반영하며 전체 인프라 구성이 동일하게 유지될 수 있도록 관리하려고 합니다.
